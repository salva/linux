...
#define MAX_BUFF 100
static int record_connection(char *host, char *port, char *busid, int vhci_ix, int rhport)
{
	int fd;
...
static int import_device(int sockfd, struct usbip_usb_device *udev, int *pvhci_ix, int *pport)
{
	uint32_t speed = udev->speed;
        ...
        udev_list_entry_foreach(entry, list) {
                ...
                if (usbip_vhci_driver_open_path(udev_list_entry_get_name(entry)) < 0)
                        continue;
                 ...
                 * condition, we retry on EBUSY errors */
                while (1) {
...
static int query_import_device(int sockfd, char *busid, int *pvhci_ix, int *pport)
{
	int rc;

			    ...
			    const char *buf, size_t count)
{
	...
	if (port >= VHCI_HC_PORTS)
                ret = vhci_port_disconnect(vhci->vhci_hcd_ss, port - VHCI_HC_PORTS);
	else
...
int vhci_init_attr_group(struct vhci_hcd *vhci_hcd)
{
        ...
        vhci_attrs = kcalloc(1, sizeof(struct vhci_attrs), GFP_KERNEL);
        if (vhci_attrs == NULL)
                return -ENOMEM;

        attrs = kmalloc(sizeof(struct attribute *) * 5, GFP_KERNEL);
        if (attrs == NULL) {
                kfree(vhci_attrs);
                return -ENOMEM;
        }
        ...
        vhci_attrs->dev_attr_status.attr.attr.mode = S_IRUGO;
        ...
        vhci_attrs->dev_attr_attach.attr.attr.mode = S_IWUSR;
        ...
        vhci_attrs->dev_attr_detach.attr.attr.mode = S_IWUSR;
        ...
        vhci_attrs->dev_attr_nports.attr.attr.mode = S_IRUGO;
